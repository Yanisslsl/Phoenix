...
namespace Phoenix
{
    ...
    void PHOENIX_API Entity::AddComponent<SpriteComponent>(SpriteComponent component)
    {
        if(component.spriteType == Cube)
        {
            if(component.textureFilePath.empty())
            {
                ...
                Application::Get().GetRenderer()->CreateCube(m_name, Colors::GetColor(component.colorCode), glm::mat4(1));
    ...
    void Entity::SetRotation(float rotation, glm::vec3 axis)
    {
    ...
    {
        const glm::vec3 scaleVec = glm::vec3((float)scale, (float)scale, float(scale));
    ...
    glm::mat4 Entity::GetRotationMatrix() const
    {
        ...
        const auto rotationAxis = Application::Get().GetSubSystem<TransformSubsytem>()->GetTransformRotationAxis(m_EntityHandle);
        return rotate(glm::mat4(1.0f), glm::radians(rotation), rotationAxis);
    }
